...
- **Options** — capitals can be turned off entirely (flag → country only, no bonus stage), and **multiple choice** can be turned on (off by default): answer by picking one of 6 options instead of typing — easier and kid-friendly, with decoys drawn from the same region and similar difficulty so they're plausible rather than obviously wrong. Works in solo and duels (steals get a fresh grid, and the answer isn't revealed until the steal is over). Both choices are remembered.
