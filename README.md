...
- **2 Players** — duel mode: enter two names and a target score (your choice, min 10, default 100). Players alternate full turns (one flag each), and the game can only end on a **round boundary**, so both players always get the same number of turns. First to be ahead at or above the target after a completed round wins; a tie at the target goes to **sudden death** (more rounds until someone leads). If the deck runs out, the tiebreaker chain is points → capitals guessed → hints left → tie. No lives; each player gets their own 2 hints. **Fair rounds**: each round deals both players flags from the same point tier (adjacent tier at worst). Duel results don't enter the solo high-score table.
- **Options** — capitals can be turned off entirely (flag → country only, no bonus stage); the choice is remembered.
